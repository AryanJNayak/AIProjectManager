...
import type { ExtractionResponse, Task, TaskFilters as TaskFiltersType, TaskPriority, TaskStatus } from './types/task';

function App() {
  const [notes, setNotes] = useState('');
  const [tasks, setTasks] = useState<Task[]>([]);
  const [filters, setFilters] = useState<TaskFiltersType>({ owner: '', status: 'All', priority: 'All' });
  ...
  const visibleTasks = useMemo(() => {
    return tasks.filter((task) => {
      const ownerMatch = filters.owner.trim().length === 0 || task.owner.toLowerCase().includes(filters.owner.toLowerCase());
  ...
  const handleExtract = async () => {
    if (!notes.trim()) {
      setInsight('Add a few notes first so the assistant has something to analyze.');
      return;
    }
  ...
  return (
    <div className="dashboard-shell">
      <header className="hero-panel">
        <div>
          ...
          <p className="hero-copy">
            Guide your team with AI-assisted extraction, live task tracking, and focused delivery views.
          </p>
        </div>
      ...
      <div className="content-grid">
        <NoteComposer value={notes} onChange={setNotes} onExtract={handleExtract} isLoading={isLoading} />
        <TaskFilters filters={filters} onChange={setFilters} />
      </div>

      {insight ? <div className="insight-card">{insight}</div> : null}

      <TaskBoard tasks={visibleTasks} onStatusChange={handleStatusChange} onPriorityChange={handlePriorityChange} />
    </div>
  );
}
